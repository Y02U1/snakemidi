...
class MIDIRhythm:
    """
    Singola istanza di un .mid in Python con metodi per la manipolazione ritmica
    """
    ...
    def proto(self):
        staves = []
        for track in self.split_midi():
            notes = self.read_track(track, self.file.ticks_per_beat)
            if len(notes[0]) is 0 and len(notes[1]) is 0:
                pass
            else:
                x, y = self.compute_values(notes, self.file.length, resolution=self.tempo_refres(self.get_tempo(track)))
                staves.append([x, y])
        return staves
    ...
    def read_track(self, track, tick_per_beat):
        """
        Leggi il track
        ...
        for msg in track:
            ...
            if str(msg.type) == "note_on":
                ...
                notes[2].append(self.get_tempo(track))
        return notes

    def compute_values(self, notes, length, resolution=100):
        """
        ...
        for i in range(len(notes[0])):
            ...
            try:
                ...
                note_res = round(self.tempo_deltares(notes[2][i], delta, resolution))
            ...
            if notes[0][i] is 'n':
                ...
                for x in x_range:
                    ...
                    y_for_note.append(self.y_eased(x))
    ...
    def tempo_deltares(self, tempo, delta, ref_res):
        """
